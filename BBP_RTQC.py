...
def QC_wmo(iwmo, PLOT=False, SAVEPLOT=False, SAVEPKL=False, VERBOSE=False):
    ...
    def test_tests(ia):
        ...
        assert np.all(QC_FLAGS_OUT == a[ia]['output']['flags_out']) and myfunc(
            tests[code] + ' / ' + a[ia]['specifics'] + ': test succeded.'), 'Assertion error for ' + tests[code]
    ...
    def BBP_Noisy_profile_test(BBP, BBPmf1, PRES, QC_Flags, QC_1st_failed_test,
        ...
        if len(innan) > 10:  # if we have at least 10 points in the profile
            ...
            res[innan] = np.abs(BBP[innan] - BBPmf1[innan])
            ioutliers = np.where(abs(res[innan][iPRES]) > B_RES_THRESHOLD)[
                0]  # index of where the rel res are greater than the threshold

            if len(ioutliers) / len(
                    innan) >= B_FRACTION_OF_PROFILE_THAT_IS_OUTLIER:  # this is the actual test: are there more than a certain fraction of points that are noisy?
                ISBAD = ioutliers
    ...
    def BBP_Missing_Data_test(BBP, PRES, QC_Flags, QC_1st_failed_test,
        ...
        if np.any(np.nonzero(bin_counts < MIN_N_PERBIN)[0]):
            ...
            if nonempty.size != 0:
                test_bin = np.linspace(0, nonempty[-1], nonempty[
                    -1] + 1)  # create array with consecutive indices from 0 to the last non-empty element of nonempty
                ...
                # and if not all bins contain data
                # and if max(PRES) is greater than the PRES in the deepest non-empty bin
                elif (np.all(test_bin == nonempty)) & (nonempty[-1] < len(bins) - 1) & (
                        np.nanmax(PRES) >= bins[nonempty[-1]]):  # with test
                    if VERBOSE: print("shallow profile due to missing data: QC=" + str(QC_all[1]))
                    QC = QC_all[1]

                # check if max(PRES)<maxPresbin to decide if this was a profile that was programmed to be shallow
                elif (np.all(test_bin == nonempty)) & (nonempty[-1] < len(bins) - 1) & (
                        np.nanmax(PRES) < bins[nonempty[-1]]):
                    if VERBOSE: print(
                        "shallow profile (maxPRES=" + str(np.nanmax(PRES)) + ") dbars. Need more checks...")
                    ...
                    if not iGT200.any():  # with test    # if there are no data deeper than 200 dbars, then set QC=3 without checking what the values are
                        ...
                        # if there are no non-NaN BBP values
                        if (not medBBPGT200) | np.isnan(medBBPGT200):  # with test
                            if VERBOSE: print("----shallow profile (maxPRES=" + str(
                                np.nanmax(PRES)) + " dbars) no BBP data below 200 dbars) : QC=" + str(QC_all[1]))
                            QC = QC_all[1]

                        elif medBBPGT200 < E_DEEP_BBP700_THRESH:  # with test
                            # set ISBAD so that the test does not fail when it's a shallow profile without high-deep values
                            if VERBOSE: print(
                                "----shallow profile (maxPRES=" + str(np.nanmax(PRES)) + " dbars)) : QC=" + str(
                                    QC_all[0]))
                            ISBAD = 0
                            QC = QC_all[0]
                            ...
                            if VERBOSE: print("----shallow profile (maxPRES=" + str(
                                np.nanmax(PRES)) + " dbars) with high deep values (medBBPGT200=" + str(
                                medBBPGT200) + "): QC=" + str(QC_all[1]))
                            QC = QC_all[1]
    ...
    def plot_failed_QC_test(BBP, BBPmf1, PRES, ISBAD, QC_Flags, QC_1st_failed_test, QC_TEST_CODE,
        ...
        if SAVEPLOT:
            if VERBOSE:
                print("saving plot...")
            fname = DIR_PLOTS + "/" + fn.split('/')[-3] + "/" + fn.split('/')[-4] + "_" + fn.split('/')[
                -1] + "_" + QC_TEST_CODE + ".png"
            fig.savefig(fname, dpi=75)
    ...
    def rd_WMOmeta(iwmo, VERBOSE):
        ...
        if not np.any(ds_config.SENSOR.astype('str').str.contains('BACKSCATTERINGMETER_BBP700')):
            ...
            iBBPsensor = np.where(ds_config.SENSOR.astype('str').str.contains('BACKSCATTERINGMETER_BBP700'))[0][
                0]  # find index of BBP meter
            SENSOR_MODEL = ds_config.SENSOR_MODEL[iBBPsensor].astype('str').values
            SENSOR_MAKER = ds_config.SENSOR_MAKER[iBBPsensor].astype('str').values
            SENSOR_SERIAL_NO = ds_config.SENSOR_SERIAL_NO[iBBPsensor].astype('str').values

            # read PRE-DEPLOYMENT calibration coefficients
            if re.search("BACKSCATTERING700", str(ds_config.PREDEPLOYMENT_CALIB_COEFFICIENT.astype(
                    'str').values)):  # check that CAL COEFFS are stored
                iBBP700cal = \
                np.where(ds_config.PREDEPLOYMENT_CALIB_COEFFICIENT.astype('str').str.contains('BACKSCATTERING700'))[0][
                    0]
                calcoeff_string = ds_config.PREDEPLOYMENT_CALIB_COEFFICIENT[iBBP700cal].astype('str').values
                calcoeff_string = np.char.strip(calcoeff_string).item()
    ...
    def rd_BBP(fn_p, miss_no_float, ds_config, VERBOSE=False):
        ...
        # find N_PROF where the BBP700 data are stored
        tmp_bbp = ds.BBP700.values  # note that BBP700[N_PROF,N_LEVELS]
        tmp = [np.any(~np.isnan(tmp_bbp[i][:])) for i in range(
            tmp_bbp.shape[0])]  # find which of the different columns of tmp_bbp has at least one non-NaN element
        if np.any(tmp):
            N_PROF = np.where(tmp)[0][0]
        else:
            if VERBOSE: print("this profile has less than 5 data points: skipping ")
        ...
        # find Park Pressure in META file
        iParkPres = np.where(ds_config.CONFIG_PARAMETER_NAME.astype('str').str.contains('CONFIG_ParkPressure_dbar'))[0][
            0]
        PARK_PRES = ds_config.CONFIG_PARAMETER_VALUE.values[i_miss_no, iParkPres]
        if np.isnan(PARK_PRES):
            # assume PARK_PRES=1000 dbars
    ...
    def QC_wmo(iwmo, PLOT=False, SAVEPLOT=False, SAVEPKL=False, VERBOSE=False):
        ...
        # read meta file
        [ds_config, SENSOR_MODEL, SENSOR_MAKER, SENSOR_SERIAL_NO, PLATFORM_TYPE,
         miss_no_float, DARK_BACKSCATTERING700, SCALE_BACKSCATTERING700, KHI_BACKSCATTERING700] = rd_WMOmeta(iwmo,
                                                                                                             VERBOSE)
        ...
        for ifn_p, fn_p in enumerate(fn_single_profiles):
            ...
            # read BBP and PRES + other data from profile file
            [PRES, BBP700, JULD, LAT, LON, BBP700mf1, miss_no_prof, PARK_PRES, maxPRES, innan] = rd_BBP(fn_p,
                                                                                                        miss_no_float,
                                                                                                        ds_config,
                                                                                                        VERBOSE)
            if np.any(PRES == -12345678):
            ...
            # GLOBAL-RANGE TEST for BBP700
            BBP700_QC_flag, BBP700_QC_1st_failed_test = BBP_Global_range_test(BBP700, BBP700mf1, PRES, BBP700_QC_flags,
                                                                              BBP700_QC_1st_failed_test, fn_p, PLOT,
                                                                              SAVEPLOT, VERBOSE)

            # SURFACE-HOOK TEST for BBP700
            BBP700_QC_flag, BBP700_QC_1st_failed_test = BBP_Surface_hook_test(BBP700, BBP700mf1, PRES, BBP700_QC_flags,
                                                                              BBP700_QC_1st_failed_test, fn_p, PLOT,
                                                                              SAVEPLOT, VERBOSE)

            # PARKING-HOOK TEST for BBP700
            BBP700_QC_flag, BBP700_QC_1st_failed_test = BBP_Parking_hook_test(BBP700, BBP700mf1, PRES, maxPRES,
                                                                              PARK_PRES, BBP700_QC_flags,
                                                                              BBP700_QC_1st_failed_test, fn_p, PLOT,
                                                                              SAVEPLOT, VERBOSE)

            # BBP_NOISY_PROFILE TEST
            BBP700_QC_flag, BBP700_QC_1st_failed_test, rel_res = BBP_Noisy_profile_test(BBP700, BBP700mf1, PRES,
                                                                                        BBP700_QC_flags,
                                                                                        BBP700_QC_1st_failed_test, fn_p,
                                                                                        PLOT, SAVEPLOT, VERBOSE)
            if np.any(BBP700_QC_1st_failed_test["B"] != '0'):
                plot_failed_QC_test(BBP700, BBP700mf1, PRES, BBP700 * np.nan, BBP700_QC_flags,
                                    BBP700_QC_1st_failed_test,
                                    '0', fn_p, SAVEPLOT, VERBOSE)

            # HIGH_DEEP_VALUES TEST
            BBP700_QC_flag, BBP700_QC_1st_failed_test = BBP_High_Deep_Values_test(BBP700mf1, PRES, BBP700_QC_flags,
                                                                                  BBP700_QC_1st_failed_test, fn_p, PLOT,
                                                                                  SAVEPLOT, VERBOSE)

            # MISSING_DATA TEST
            BBP700_QC_flag, BBP700_QC_1st_failed_test = BBP_Missing_Data_test(BBP700, PRES, BBP700_QC_flags,
                                                                              BBP700_QC_1st_failed_test, fn_p, PLOT,
                                                                              SAVEPLOT, VERBOSE)

            # NEGATIVE NON-SURFACE TEST
            BBP700_QC_flag, BBP700_QC_1st_failed_test = BBP_Negative_nonsurface_test(BBP700, PRES, BBP700_QC_flags,
                                                                                     BBP700_QC_1st_failed_test, fn_p,
                                                                                     PLOT, SAVEPLOT, VERBOSE)
        ...
        if SAVEPKL:
            # save results in pickled file (https://www.datacamp.com/community/tutorials/pickle-python-tutorial)
            fname = DIR_PLOTS + fn_p.split('/')[-3] + "/" + fn_p.split('/')[-4] + "_" + \
                    fn_p.split('/')[-1].split('.')[0].split('_')[0] + ".pkl"
            fnout = open(fname, 'wb')
            pickle.dump(all_PROFS, fnout)
            fnout.close()
